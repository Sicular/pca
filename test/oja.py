...
def oja_batch(X, k, max_iter = 1, batch_size = 1, tol=1e-9):
    ...
    for i in range(max_iter):
        ...
        for j in range(n_batch):
            ...
            his.append(b)
            # display3D_remote(X, mu, w.T, True)
    f = plt.figure()
    ax = f.add_subplot()
    # print(his)
    ax.plot(his)
...
def ada_oja_method(X, k, max_iter, mu, dependency, tol=1e-4):
    ...
    for i in range(max_iter):
        ...
        print(i)
        # print(f"old: \n{old_w}\nnew:\n{w}")
        # if np.sum(np.abs(old_w - w) > tol) == 0:
        # print(f"stop at {i}")
        # break
        display3D(X.T, mu, w.T, True)
    # display2D(X.T, mu, w.T)
    return w.T.dot(X)
...
# input = csr.get_multirow(0, 10000)[:,:]
...
# t1 = time()
# pca = oja_batch(input, k, 100, mu, dependency_kwargs)
# t2 = time()
# print(t2 - t1)
# # print(pca.shape)
...
# # print("exact: ", np.cov(pca_ex))
# print("exact: ", np.sort(np.diag(np.cov(pca_ex))))
# print("exact eig: ", np.sort(eig_val[:k]))
